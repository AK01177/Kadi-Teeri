...
class ConnectionManager:
    ...
    async def add(self, room_id: str, player_id: str, ws: WebSocket):
        ...
        if is_new_room and redis_client.redis_client:
    ...
    async def send_to(self, player_id: str, message: dict):
        ...
        if ws:
            ...
            # But normally we broadcast to the room, and client filters it, or we rely on personalized state.
            pass
    ...
    async def broadcast(self, room_id: str, message: dict, exclude: Optional[str] = None):
        ...
        if redis_client.redis_client:
            ...
            channel_name = f"room:events:{room_id}"
            await redis_client.redis_client.publish(channel_name, json.dumps(payload))
        else:
